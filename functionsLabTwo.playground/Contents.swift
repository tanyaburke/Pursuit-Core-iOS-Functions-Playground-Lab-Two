...
func average(numArray : [Double]) -> Double {
    var averageOfArray = 0.0
    var numAvg = 0.0
    for num in numArray{
        numAvg += num
    }
    averageOfArray = numAvg / Double(numArray.count)
...
for (input, expectedOutput) in testCasesOne {
    let output = average(numArray: input)
...
func frequencyDictionary(of stringIn: String) -> [Character:Int]{
...
func fizzBuzz(upto: Int) -> [String]{
    ...
    for num1 in 1...upto {
        if (num1 % 5 == 0 && num1 % 3 == 0){
         finalNum.append("FizzBuzz")
        }else if num1 % 5 == 0 {
            finalNum.append("Buzz")
        }else if  num1 % 3 == 0 {
            finalNum.append("Fizz")
        } else {
            finalNum.append(String(num1))
        }
}
     return finalNum
}
...
func value (compareAverageTo: Double , arrayDoubles: [Double] ) -> Bool{
